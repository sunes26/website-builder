...
버전: v5.0 (다중 블록 선택 기능 완성) 🎉  
작성일: 2025년 11월  
최종 업데이트: 2025년 11월 21일 - Ctrl+클릭, Shift+클릭 문제 해결 완료
...
9. [다중 블록 선택 시스템](#9-다중-블록-선택-시스템) ✅ 완성
10. [블록 시스템](#10-블록-시스템)
...
- 🎯 **다중 블록 선택**: Figma처럼 Ctrl+클릭, Shift+클릭으로 여러 블록 선택 ✅ 완성
...
3. Ctrl+A: 모든 블록 선택
4. Delete: 선택된 블록 일괄 삭제
5. Ctrl+C/V: 다중 복사/붙여넣기
6. Ctrl+X: 다중 잘라내기
```
...
└── 📁 src/
    ...
    │
    ├── 📁 store/                      # 상태 관리
    │   └── 📄 builderStore.ts         # Zustand 스토어 ✅ v5.0 완성
    │
    ├── 📁 types/                      # TypeScript 타입
    │   └── 📄 index.ts                # 모든 타입 정의 ✅ v5.0 완성
    │
    └── 📁 utils/                      # 유틸리티
...
| `BlockPanel.tsx` | 블록 목록 패널 | ⭐⭐⭐ | - |
| `SettingsPanel.tsx` | 설정 패널 | ⭐⭐ | - |
| `Canvas.tsx` | Ctrl+클릭, Shift+클릭 | ⭐⭐⭐ | ✅ v5.0 |
| `BlockRenderer.tsx` | 체크박스 + 선택 표시 | ⭐⭐⭐ | ✅ v5.0 |
| `ImageUploader.tsx` | 이미지 업로드 | ⭐⭐⭐ | - |
| `MindMapView.tsx` | 마인드맵 뷰 | ⭐⭐⭐ | - |
| `CustomEdge.tsx` | 드래그 가능한 엣지 | ⭐⭐⭐ | - |
| `PreviewNavBar.tsx` | 미리보기 네비게이션 | ⭐⭐ | - |
| `builderStore.ts` | 상태 관리 | ⭐⭐⭐ | ✅ v5.0 |
| `types/index.ts` | 타입 정의 (SelectionMode 등) | ⭐⭐⭐ | ✅ v5.0 |
| **PropertiesPanel/** | | | |
...
v5.0에서 완성된 기능으로, Figma처럼 여러 블록을 동시에 선택하고 편집할 수 있습니다.
...
- [x] Canvas.tsx 구현 (Ctrl+클릭, Shift+클릭 완전 작동) ✅
- [x] BlockRenderer.tsx 구현 (체크박스, 선택 표시) ✅
- [x] Toolbar.tsx 업데이트 (선택 개수 배지) ✅
- [x] PropertiesPanel 업데이트 (다중 선택 패널) ✅
...
#### 예시 4: 텍스트/이미지 위에서 Ctrl+클릭 ✅
```
1. 히어로 블록의 제목 텍스트 Ctrl+클릭
   → 블록 전체가 선택됨 (텍스트 선택 안 됨)
2. 콘텐츠 블록의 이미지 Shift+클릭
   → 범위 선택 작동
```
...
#### 이벤트 캡처 방식
```typescript
// Canvas.tsx - Ctrl+클릭, Shift+클릭 처리
<div
...
**핵심**: `onClickCapture`를 사용하여 이벤트 캡처 단계에서 Ctrl/Shift 키를 먼저 감지합니다. 이를 통해 자식 요소(텍스트, 이미지 등)를 클릭해도 블록 선택이 작동합니다.

### 9.10 적용 방법

상세한 적용 방법은 다음 문서를 참조하세요:
...
```bash
# 1. 백업
cp src/components/Canvas.tsx src/components/Canvas.tsx.backup
cp src/components/Toolbar.tsx src/components/Toolbar.tsx.backup
cp src/components/PropertiesPanel/index.tsx src/components/PropertiesPanel/index.tsx.backup

# 2. 적용
cp outputs/Canvas.tsx src/components/
cp outputs/Toolbar.tsx src/components/
cp outputs/PropertiesPanel-index.tsx src/components/PropertiesPanel/index.tsx
...
**원인:**
- 자식 요소에서 `e.stopPropagation()` 실행
- 블록 클릭 이벤트가 차단됨

**해결:**
```typescript
// Canvas.tsx의 onClickCapture 확인
...
**적용:**
- 제공된 `Canvas.tsx`를 프로젝트에 복사하세요
- `CTRL_CLICK_TEST_GUIDE.md` 참조

#### Q2: Ctrl+A를 눌러도 텍스트가 선택돼요
...
{selectedBlockIds.length > 1 && (
  <div className="mr-2 px-3 py-1 bg-blue-100 text-blue-700 rounded-full">
...
- ✅ 다중 블록 선택 시스템 100% 완성
  ...
  - 선택 개수 배지 표시
  - 다중 선택 패널
  - **Ctrl+클릭, Shift+클릭 완전 작동** ✅
    - 텍스트/이미지/버튼 위에서도 작동
    - onClickCapture로 이벤트 캡처 단계 처리
...
Ctrl+A / Cmd+A        → 전체 선택
Escape                → 선택 해제
Ctrl+클릭              → 블록 추가/제거 (텍스트/이미지 위에서도 작동) ✅
Shift+클릭             → 범위 선택 (어떤 요소 위에서도 작동) ✅
...
기술적 구현:
- onClickCapture로 이벤트 캡처 단계에서 Ctrl/Shift 키 감지
- 자식 요소(텍스트, 이미지) 위에서도 블록 선택 가능
- 일반 클릭 시 요소 선택 정상 작동
```
...
*이 문서는 지속적으로 업데이트됩니다.*  
*최종 업데이트: 2025년 11월 21일 (v5.0 - 다중 블록 선택 시스템 100% 완성)*